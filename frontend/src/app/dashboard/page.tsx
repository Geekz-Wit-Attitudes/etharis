...
import { Plus, Handshake, TrendingUp, DollarSign, AlertCircle, CheckCircle, Wallet } from 'lucide-react'
...
export default function Dashboard() {
  const {push} = useRouter()
    const { user, deals, balance } = useEtharisStore();

    // Pastikan ini adalah dashboard Brand
    if (user?.role !== 'brand') {
        push("/auth/login")
        // Redirect atau error state jika salah role
        return <div className="min-h-screen bg-[var(--color-light)]"></div>;
    }
